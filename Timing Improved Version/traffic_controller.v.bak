...
module traffic_controller(
    ...
    // Count thresholds for each sub-state at 50MHz clock
    parameter [26:0] COUNT_THRESHOLD_RY = 27'd50_000_000; // 1 second
    parameter [26:0] COUNT_THRESHOLD_G = 27'd200_000_000; // 4 seconds
    parameter [26:0] COUNT_THRESHOLD_Y = 27'd50_000_000; // 1 second
    parameter [26:0] COUNT_THRESHOLD_R = 27'd50_000_000; // 1 second

    reg [26:0] current_threshold;
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            sub_state <= RY;
            count <= 27'b0;
            current_threshold <= COUNT_THRESHOLD_RY;
        end else begin
            if (count >= current_threshold - 1) begin
                count <= 27'b0;
                if (sub_state == R) begin
                    ...
                    case (sub_state)
                        RY: current_threshold <= COUNT_THRESHOLD_G;
                        G: current_threshold <= COUNT_THRESHOLD_Y;
                        Y: current_threshold <= COUNT_THRESHOLD_R;
                        default: current_threshold <= COUNT_THRESHOLD_RY;
                    endcase
                end
            end else begin
                count <= count + 1;
